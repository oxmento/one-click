...
xtls_mode_logic(){
    if [ "${SS_VERSION}" != "ss-rust" ]; then
        ...
        _echo -u "${Tip} 仅当shadowsocks When the password is set to NONE, the XTLS mode is effective效。"
        _echo -d "       Shadowsocks Version and encryption methods have been changed to change${Green}${SS_VERSION}${suffix} 和 ${Green}${shadowsockscipher}${suffix}"
    fi
    mode=xtls
    get_input_inbound_port 443
    firewallNeedOpenPort="${INBOUND_PORT}"
...
install_prepare_libev_gost_plugin(){
    generate_menu_logic "${GOST_PLUGIN_TRANSPORT_MODE[*]}" "Transmission mode" "1"
