...
install_caddy_v1(){
    ...
    echo && echo -e " Caddy Use command:${CADDY_CONF_FILE}
 Log file:/var/log/caddy-error.log | /var/log/caddy-access.log
 Instructions for use：service caddy start | stop | restart | status
 Or use:/etc/init.d/caddy start | stop | restart | status
${Info} Caddy The installation is complete！"
}
...
install_caddy_v2(){
    ...
    [ -z ${caddy_ver} ] && _echo -e "Obtain caddy 最新版本失败." && exit 1
 ...
 Instructions for usage: service caddy start | stop | restart | status
 Or use:/etc/init.d/caddy start | stop | restart | status
${Info} Caddy2 The installation is complete!"
}
