...
judge_current_version_num_is_none_and_output_error_info(){
    ...
    [ -z ${currentVersion} ] && _echo -e "Obtain${appName}The current version number fails, exit and run." && exit 1
}
...
judge_not_update_and_output_point_info(){
    ...
    if ! check_latest_version ${currentVersion} ${latestVersion}; then
        echo -e "${Point} ${appName}It is currently the latest version${latestVersion}No need to update."
        exit 0
    fi
}
...
judge_not_update_when_simple_tls_is_specified_version(){
    ...
    if [[ -e ${SIMPLE_TLS_BIN_PATH} ]]; then
        if ! check_latest_version "0.4.7" ${currentVersion}; then
            echo -e "${Point} ${appName}The current version is${currentVersion}And the following version，Not compatible with the latest version, the script does not provide updates."
            exit 0
        fi
    fi
}
...
update_download(){
    ...
    _echo -i "detected${downloadFileName}There are new versions, start downloading."
    ...
    _echo -i "${downloadFileName}Download the download, wait for installation."
}
...
update_completed(){
    ...
    _echo -i "${appName}Successfully upgraded to the latest version${latestVersion}" && echo
    install_cleanup
}
...
update_caddy_v1(){
    ...
    _echo -i "current version：${currentVersion}"
    read -p "Whether to enforce the installation caddy(default: n) [y/n]: " yn
    [ -z "${yn}" ] && yn="N"
    case "${yn:0:1}" in
        y|Y)
            ...
            _echo -i "Cover version：${latestVersion}"
            ;;
        *)
            _echo -i "Skip forced installation。"
            ;;
    esac
...
update_caddy_v2(){
    ...
    _echo -i "detected${appName}There are new versions, start downloading."
    caddyVerFlag="${versionMark}"
    update_install "${packageName}" "${shFileName}" "${calledFuncName}"
