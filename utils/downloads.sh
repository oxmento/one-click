download(){
    ...
    if [ -e ${1} ]; then
        ...
        echo "${filename} There is no existence in the current directory, Start download now."
        wget --no-check-certificate -c -t3 -T60 -O ${1} ${2}
        if [ $? -ne 0 ]; then
            _echo -e "下载 ${filename} fail."
            exit 1
        fi
    fi
}
...
judge_latest_version_num_is_none_and_output_error_info(){
    ...
    [ -z ${latestVersion} ] && _echo -e "Get $ {AppName} the latest version number failed, withdrawing and running." && exit 1
}
