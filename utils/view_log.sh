dynamic_display_log(){
    ...
    echo -e "${Green}End the visit：${suffix}Press Ctrl + C"
    echo -e "${Green}Log path：${suffix}${args}"
    echo -e "${Separator_1}${Separator_1}"
    tail -f ${args}
}
...
show_log(){
    ...
    if [[ -e ${ssLog} ]] && [[ -e ${kcptunLog} ]]; then
        ...
        if [[ -e ${ssLog} ]]; then
            ...
            echo && echo -e "${Point} There are no log files at present, please try after the log file is generated。" && echo
        fi
    fi
}
