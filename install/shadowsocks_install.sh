install_shadowsocks_libev(){
    ...
    if [ $? -eq 0 ]; then
        ...
        _echo -i "shadowsocks-libev Successful installation."
    else
        _echo -e "shadowsocks-libev installation failed."
        install_cleanup
        exit 1
    fi
    popd > /dev/null 2>&1
...
install_shadowsocks_rust(){
    ...
    if [ $? -eq 0 ]; then
        ...
        _echo -i "shadowsocks-rust Successful installation."
    else
        _echo -e "shadowsocks-rust installation failed."
        install_cleanup
        exit 1
    fi
    popd > /dev/null 2>&1
...
install_go_shadowsocks2(){
    ...
    if [ $? -eq 0 ]; then
        ...
        _echo -i "go-shadowsocks2 Successful installation."
    else
        _echo -e "go-shadowsocks2 installation failed."
        install_cleanup
        exit 1
    fi
    popd > /dev/null 2>&1
